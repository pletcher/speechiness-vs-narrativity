...
from ARISTOPHANES_compare_messenger_speeches import (
...
RESULTS_CSV = ROOT_DIR / "csv" / "ARISTOPHANES_messenger_speech_narrativity.csv"
...
def plot_line_position(df: pd.DataFrame, plays: list[dict]) -> None:
    ...
    for play in sorted(df["play"].unique()):
        ...
        out_path = FIG_DIR / f"ARISTOPHANES_line_position_{play}.png"
...
def plot_distribution(df: pd.DataFrame) -> None:
    ...
    FIG_DIR.mkdir(exist_ok=True, parents=True)
    out_path = FIG_DIR / "ARISTOPHANES_distribution.png"
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    print(f"Wrote {out_path}")
